...
def main():
	...
	parser.add_argument('--cMap',  help='Optional way of mapping the "color" variable in the data matrix to a color code. If your colormap file includes headers, provide 3 comma-separated values: map file, key header, value header. If not, just provide a map file, it is expected the first column is the value and the second is the color.')
	parser.add_argument('--cMapCat',  help='Optional way of mapping the "color" variable in the data matrix to another categorical variable. If your colormap file includes headers, provide 3 comma-separated values: map file, key header, value header. If not, just provide a map file, it is expected the first column is the value and the second is the color.')
	parser.add_argument('--xLog', default=False, type=float, help="Use if you want x-axis to be shown on a log-scale, base 10. Argument provided should be a float to add to the y values before calculating the log value.")
	parser.add_argument('--yLog', default=False, type=float, help="Use if you want y-axis to be shown on a log-scale, base 10. Argument provided should be a float to add to the y values before calculating the log value.")
	parser.add_argument('--xLog2', default=False, type=float, help="Use if you want x-axis to be shown on a log-scale, base 2. Argument provided should be a float to add to the y values before calculating the log value.")
	parser.add_argument('--yLog2', default=False, type=float, help="Use if you want y-axis to be shown on a log-scale, base 2. Argument provided should be a float to add to the y values before calculating the log value.")
...
#	parser.add_argument('--height', default=4, type=int, help="Figure height.")
	...
	if opts.cMap:
		if "," in opts.cMap:
			...
			opts.cMap = io.fileDict(opts.cMap, header=False)

	if opts.cMapCat:
		if "," in opts.cMapCat:
			mapF, kHead, vHead = opts.cMapCat.split(",")
			opts.cMapCat = io.fileDictHeader(mapF, kHead, vHead)
		else:
			opts.cMapCat = io.fileDict(opts.cMapCat, header=False)
...
def scatter(dd, opts):
	...
	if opts.color:
		...
		if opts.cMap:
			for n, c in opts.cMap.items():
				colD[n] = c
			sPoints = ax.scatter(dd[opts.xHead], dd[opts.yHead], c=[opts.cMap[a] for a in dd[opts.color]], alpha=opts.alpha, s=opts.markerSize)
