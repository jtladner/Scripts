...
#In version 3.0, added option for reverse complementing an orf
    #This will happen anytime the start coordinate > the stop coordinate
...
def make_beast_nexus(opts):
    ...
    for start, end in coding_coords:
        for i in range(len(seqs)):
            #If ORF needs to be reverse complemented
            if start>end: coding_seqs[i]+=rev_comp(seqs[i][end-1:start])
            else: coding_seqs[i]+=seqs[i][start-1:end]
