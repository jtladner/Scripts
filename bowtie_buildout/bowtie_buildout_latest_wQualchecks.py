...
def check_quality_coverage(pileup, ref, opts):
    ...
    for chrom, count in num_bases_lack_qual_cov.iteritems():
        opts.fout_sum.write('\t%s\t%d\n' % (chrom, count))
...
def make_final_contigs(ref_dict, pos_dict, weak_dict):
    ...
    for contig in ref_dict.iterkeys():
    ...
    for contig, seq_list in new_dict.iteritems():
...
def plot_coverage(cov_files):
    ...
    for file in cov_files:
        ...
        for chrom in pos_list_dict.iterkeys():
...
def bowtie_build(opts):
    ...
    print >> sys.stderr, '%d changes made in iteration %d' % (num_changes, iter_num)
...
def make_consensus(pileup, ref, opts):
    ...
    write_fasta(ref_dict.keys(), ref_dict.values(), new_cons_name)
...
def pair_cov_test(sam_file, opts):
    ...
    for ref, counts in ref_dict.iteritems():
        start=0
        end=0
